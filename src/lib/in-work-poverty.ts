...
const CSV_PATH = "/data/clean_in_work_poverty.csv";

export async function loadInWorkPoverty(): Promise<IwpRow[]> {
...
export function aggregate(rows: IwpRow[], sexFilter: SexFilter): CountryYearValue[] {
  if (sexFilter !== "all") {
    return rows
...
export function aggregateBySex(rows: IwpRow[]): BySexRow[] {
...
export const DEFAULT_TREND_COUNTRIES = ["Sweden", "Germany", "France", "Spain", "Romania"];
